...
#import "CheckOverStory.h"
#import "Cutscene.h"
#import "Sakura.h"
#import "Shingo.h"
#import "Bag.h"

@implementation CheckOverStory
...
- (void)storyResult {
    ...
    [[Bag sharedBag] addItemWithClassName:@"ChocolateItem" count:1];
    [[Bag sharedBag] addItemWithClassName:@"ChipsItem" count:2];
    [[Bag sharedBag] addItemWithClassName:@"JellyItem" count:1];
    [[Bag sharedBag] addItemWithClassName:@"WaterItem" count:2];
    
    //额外补给
    if ([[System system] boolProperty:@"foodPurchased"]) {
        
        [[Bag sharedBag] addItemWithClassName:@"BreadItem" count:1];
    ...
    [[Person me] setProperty:@1 forKey:@"talkLimit"];
    [[Person partner] setProperty:@"LoungeScene" forKey:@"scene"];
    
    [[NSNotificationCenter defaultCenter] postNotificationName:K_SHOULD_RELOAD_BAGITEMACTIONS_NOTIFICATION object:nil];
    
    [[MainView sharedView] hideMyImageWithAnimation:YES];
    [[MainView sharedView] hidePartnerImageWithAnimation:YES];
    [Message hideMessage];
    
    [Story beginStoryWithClassName:@"Talk0Story"];
}
